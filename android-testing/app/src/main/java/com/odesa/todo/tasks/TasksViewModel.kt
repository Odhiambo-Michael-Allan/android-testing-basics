...
import com.odesa.todo.tasks.TasksFilterType.*
...
class TasksViewModel ( private val tasksRepository: TasksRepository ) : ViewModel() {
    ...
    private var currentFiltering = ALL_TASKS
    ...
    val empty: LiveData<Boolean> = _tasks.map { it.isEmpty() }
    ...
    private fun filterTasks( tasksResult: Result<List<Task>> ): LiveData<List<Task>> {
        // TODO: This is a good case of liveData builder. Replace when stable.
        val result = MutableLiveData<List<Task>>()
        if ( tasksResult is Success ) {
            isDataLoadingError.value = false
            viewModelScope.launch {
                result.value = filterTasks( tasksResult.data, currentFiltering )
            }
        } else {
